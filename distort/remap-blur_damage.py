...
print("damaging...")
stained = np.zeros_like(newf)
side = max(im.shape[0], im.shape[1])
for i in range(0,50):
    x = int(r.normalvariate(side/2, side/2))
    y = int(r.normalvariate(side/2, side/2))
    a1 = abs(int(r.normalvariate(side/2, side/2)))
    a2 = abs(int(r.normalvariate(side/2, side/2)))
    angle = abs(int(r.normalvariate(180, 180)))
    s_angle = abs(int(r.normalvariate(180, 180)))
    e_angle = abs(int(r.normalvariate(180, 180)))
    if s_angle > e_angle:
        (s_angle, e_angle) = (e_angle, s_angle)
    delta = 1

    print("ellipse", (x,y), (a1, a2), angle, s_angle, e_angle, delta)
    pts = cv2.ellipse2Poly((x,y), (a1, a2), angle, s_angle, e_angle, delta)
    stained = cv2.fillPoly(stained, pts, 255)
    name = "ellipse_" + sys.argv[4]
    print("writing ellipse visualization %s..." % name)
    cv2.imwrite(name, stained)
